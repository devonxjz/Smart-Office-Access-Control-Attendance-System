...
import { X, Lock, Save, AlertTriangle } from 'lucide-react';
...
export function EmployeeDetailModal({ isOpen, employee, onClose, onSuccess }: EmployeeDetailModalProps) {
  ...
  const submitInfoUpdate = async () => {
    ...
    try {
      const client = new GoogleSheetsClient(import.meta.env.VITE_GAS_URL || '');
      await client.updateEmployee(employee?.['Mã NV'] || '', {
  ...
  const submitPasswordUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const client = new GoogleSheetsClient(import.meta.env.VITE_GAS_URL || '');
      await client.updatePassword(employee?.['Mã NV'] || '', passwordData.newPassword);
      setPasswordData({ newPassword: '', confirmPassword: '' });
      alert('Đã đổi mật khẩu thành công');
    } catch (err: any) {
      setPwdErrors({ submit: err.message });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleDeactivate = async () => {
    ...
    try {
      const client = new GoogleSheetsClient(import.meta.env.VITE_GAS_URL || '');
      await client.deactivateEmployee(employee?.['Mã NV'] || '');
      onSuccess();
      onClose();
    } catch (err: any) {
      setErrors({ submit: err.message });
